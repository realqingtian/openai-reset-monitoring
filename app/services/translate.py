...
async def translate_tweet(client, tweet_id: str, to: str) -> TranslateOut:
    ...
    if cached:
        return TranslateOut(id=tweet_id, to=lang, text=cached["text"],
                            provider=cached["provider"] or "", cached=True)
    ...
    return TranslateOut(id=tweet_id, to=lang, text=result["text"],
                        provider=result.get("provider") or "", cached=False)
