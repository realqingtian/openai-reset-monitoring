...
async def log_poll(account: str, source: str, ok: bool, new_tweets: int = 0,
                   error: Optional[str] = None, latency_ms: Optional[int] = None):
    async with session_factory() as s:
        s.add(Poll(ts=iso_utc(), account=account, source=source, ok=int(bool(ok)),
                   new_tweets=new_tweets, error=error, latency_ms=latency_ms))
        await s.commit()
